...
class MoralisProvider implements TokenDataProvider {
  ...
  async getSwapsByTokenAddress(
    { tokenAddress, toDate, fromDate, transactionTypes }:
      GetSwapsByTokenAddressParams,
    ...
    try {
      ...
      const response = await this.apiClients.token.get(
        `${tokenAddress}/swaps`,
        {
          searchParams,
        },
      ...
      throw handleMoralisError(
        error,
        `fetching swaps for token: ${tokenAddress}`,
      );
    }
  }
}
