...
export function createMoralisApiClient(apiToken: string): MoralisApiClients {
  ...
  try {
    ...
    throw new Error(
      `Moralis API client creation failed: ${
...
export function validateMoralisResponse(
  response: unknown,
  tokenAddress: string,
  ...
  console.log(
    `Successfully fetched ${apiResponse.result.length} swaps for token: ${tokenAddress}`,
  );
...
export function handleMoralisError(error: unknown, context: string): Error {
  if (error instanceof HTTPError) {
    ...
    switch (status) {
      case 400:
        return new Error(
          `Bad request to Moralis API: Invalid token address or parameters (${context})`,
        );
      case 401:
      ...
      case 404:
        return new Error(
          `Token not found: ${context} - token does not exist or has no swap data`,
        );
      case 429:
        return new Error(
          "Rate limit exceeded: Too many requests to Moralis API. Please try again later.",
        );
      case 500:
      case 502:
      case 503:
      case 504:
        return new Error(
          `Moralis API server error (${status}): ${statusText}. Please try again later.`,
        );
      default:
  ...
  if (error instanceof TimeoutError) {
    return new Error(
      "Request timeout: Moralis API took too long to respond. Please try again.",
    );
  }

  if (error instanceof TypeError && error.message.includes("fetch")) {
    return new Error(
      "Network error: Unable to reach Moralis API. Please check your internet connection.",
    );
  }
