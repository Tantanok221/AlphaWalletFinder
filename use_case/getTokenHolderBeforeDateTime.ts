...
import {getDefaultTokenDataProvider, TokenDataProvider} from "@/provider/index.ts";
...
export async function getTokenHolderBeforeDateTime({
  tokenAddress,
  startDate,
  endDate,
  tokenDataProvider
}: GetTokenHolderDateRangeParams) {
  // Validation: at least one date must be provided
